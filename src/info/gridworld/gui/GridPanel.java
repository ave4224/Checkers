...
public class GridPanel extends JPanel implements Scrollable,
    ...
    private static final int DEFAULT_CELL_SIZE = 72;
    ...
    protected int cellSize; // the size of each cell, EXCLUDING the gridlines
    ...
    private void drawGridlines(Graphics2D g2) {
        ...
        for (int y = miny; y <= maxy; y += cellSize + 1) // draw horizontal lines
        {
            g2.drawLine(minx, y, maxx, y);
        }
        for (int x = minx; x <= maxx; x += cellSize + 1) // draw vertical lines
        {
